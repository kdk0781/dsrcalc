/* =============================================================================
   js/report.js — 리포트 링크 빌드 + HMAC OTL + 공유 로직  VER 2026.07-RATE-FIX
   ─────────────────────────────────────────────────────────────────────────────
   ★ _OTL_SIGN_KEY 는 js/share.js / js/report-page.js 와 동일해야 합니다
   ─────────────────────────────────────────────────────────────────────────────
   ★ v2026.07-RATE-FIX 수정사항
     · 부채 상세 내역 카드의 월 납입액 계산: r_dsr (스트레스 포함) → r_base (명목)
     · 스케줄 표와 동일한 금액이 표시되도록 일관성 확보
     · 예시: 2억/4.92%/360개월
       - 이전: 원리금 1,208,117원 (6.07% 스트레스 포함) ← 버그
       - 수정: 원리금 1,063,886원 (4.92% 명목)       ← 스케줄과 일치
...
console.log('[report.js] v2026.07-RATE-FIX 로드됨');
...
function _buildReportData(phone = null) {
  ...
  items.forEach(item => {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // 원리금균등 / 원금균등 월 납입액 (리포트 섹션3 "부채 상세 내역" 카드용)
    // ★ v2026.07: r_dsr (스트레스 포함) → r_base (명목금리)로 변경
    //   이유: 부채 상세 카드의 "월 납입액"은 실제 상환할 금액을 보여주는 것이고,
    //         스케줄(report-page.js buildFullSchedule)도 R/1200 명목금리로 계산하므로
    //         두 값을 일치시켜야 함. 이전 버전은 상단 1,345,413원(스트레스 포함) /
    //         스케줄 982,709원(명목)으로 불일치하여 고객 혼란 유발.
    //   참고: 스트레스 금리는 "종합 DSR 지수" 카드에서만 사용 (DSR 산정용)
    // ─────────────────────────────────────────────────────────────────────
    const r_base  = R / 1200;
    const lvlPmt  = calcPMT(P, r_base, cat === 'jeonse' ? n : (isPurchaseLoan(cat,n) ? n : (_C.DSR_VIRTUAL_MONTHS[cat] ?? n)));
    const prinPmt1= (P / n) + P * r_base; // 원금균등 1회차
